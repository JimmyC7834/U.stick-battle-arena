...
- In Unity hub application, download Unity editor version `2021.3.16f1`. (You will need an unity account for this)
...
- Ensure editor version is `2021.3.16f1`
  - Note that it could take up to 10 mins for Unity to download and install all the dependencies, especially opening the project for the first time.
  - It could also takes up to 2 GB of storage for the files generated by Unity.
- Once project is opened goto the `GlobalManagers` scene in  `Assets/Scene` folder.
- After the scene is loaded, press `Play` button of the editor.
### Development details
- We are using `C#` for development as it is the language Unity use.
- It is highly recommand to use `Jetbrain`'s `Rider` IDE as it has great compability with Unity.
### Running the tests
...
- [x] Player pauses in the middle of the game
